...
namespace AsyncEV3MotorCommandsLib
{
    ...
    public class BrickManager : INotifyPropertyChanged
    {
        ...
        public async Task ConnectAsync()
        {
            ...
            var firstDev = BluetoothManager.EV3Devices.FirstOrDefault();
            if (firstDev == null) return;

            await ConnectAsync(firstDev.DeviceName, firstDev.COMPort);
        }

        public async Task ConnectAsync(string deviceName, string comport)
        {
            string comport2 = BluetoothManager.EV3Devices.FirstOrDefault(dev => dev.DeviceName == deviceName && dev.COMPort == comport)?.COMPort;
            if (string.IsNullOrWhiteSpace(comport2))
            {
                await BluetoothManager.FindBluetoothDevices();
                comport2 = BluetoothManager.EV3Devices.FirstOrDefault(dev => dev.DeviceName == deviceName && dev.COMPort == comport)?.COMPort;
                if (string.IsNullOrWhiteSpace(comport2))
                {
                    return;
                }
            ...
            var bc = new BluetoothCommunication(comport2);
            Brick = new Brick(bc, true);
            ...
            //await Brick.DirectCommand.PlayToneAsync(100, 200, 1000);
        }
